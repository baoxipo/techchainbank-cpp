...
void luu_du_lieu_sau_dang_ky(TaiKhoanDangKy& taikhoan) {
    ...
    // Kiểm tra nếu tài khoản đã tồn tại → ghi đè
    for (auto& tk : ds_tai_khoan) {
        if (tk["ten_nguoi_dung"] == taikhoan.ten_nguoi_dung) {
            tk = thong_tin_tai_khoan;  // Ghi đè dữ liệu
            da_ton_tai = true;
            break;
        }
    }

    // Nếu chưa có tài khoản nào trùng tên → thêm mới
    if (!da_ton_tai) {
...
int xem_so_du(TaiKhoanDangNhap& taikhoan){
    ...
    for (auto& tai_khoan : ds_tai_khoan) {
        string ten = tai_khoan["ten_nguoi_dung"];
        if (ten == taikhoan.ten_nguoi_dung) {
            if (tai_khoan["so_du"].is_number()) {
                return tai_khoan["so_du"];
            } else if (tai_khoan["so_du"].is_string()) {
                return stoi(tai_khoan["so_du"].get<string>());
            } else {
                return -1; 
            }
        }
...
string tim_ten_tu_so_tai_khoan(string so_tai_khoan){
    ifstream in(file_data);
    if(! in.is_open()){
        cout << "\t(!) Khong the tim thay file" << file_data << endl;
        return "";
    ...
    for (auto& tai_khoan : ds_tai_khoan) {
        ...
        if(stk == so_tai_khoan){
            ten_nguoi_dung = tai_khoan["ten_nguoi_dung"];
        }
    }   
...
bool chuyen_khoan(TaiKhoanDangNhap& taikhoan1, string sotaikhoan2, int so_tien_muon_chuyen) {
